...
function sendJsonForUn(data) {
    let response = fetch("http://192.168.1.221:8080/unit", {
        method: 'POST',
        headers: {
...
function getCreatTableUnit() {
    return fetch("http://192.168.1.221:8080/unit/all")
        .then((res) => res.text())
        .then((data) => {
...
function delete_row_unit(td) {
    ...
    fetch('http://192.168.1.221:8080/unit/' + tr.id, { method: 'DELETE' })
...
function saveRowUnit(button_nom) {
    ...
    if (data['unit'] != '' && data['coefficient'] != '' && data['unit_code'] != '') {
        ...
        fetch('http://192.168.1.221:8080/unit/' + row.id, {
            method: 'PUT',
            headers: {
